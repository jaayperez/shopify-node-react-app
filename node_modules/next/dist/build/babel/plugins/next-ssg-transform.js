"use strict";exports.__esModule=true;exports.default=nextTransformSsg;exports.EXPORT_NAME_GET_SERVER_PROPS=exports.EXPORT_NAME_GET_STATIC_PATHS=exports.EXPORT_NAME_GET_STATIC_PROPS=void 0;var _constants=require("../../../lib/constants");var _constants2=require("../../../next-server/lib/constants");const EXPORT_NAME_GET_STATIC_PROPS='getStaticProps';exports.EXPORT_NAME_GET_STATIC_PROPS=EXPORT_NAME_GET_STATIC_PROPS;const EXPORT_NAME_GET_STATIC_PATHS='getStaticPaths';exports.EXPORT_NAME_GET_STATIC_PATHS=EXPORT_NAME_GET_STATIC_PATHS;const EXPORT_NAME_GET_SERVER_PROPS='getServerSideProps';exports.EXPORT_NAME_GET_SERVER_PROPS=EXPORT_NAME_GET_SERVER_PROPS;const ssgExports=new Set([EXPORT_NAME_GET_STATIC_PROPS,EXPORT_NAME_GET_STATIC_PATHS,EXPORT_NAME_GET_SERVER_PROPS,// legacy methods added so build doesn't fail from importing
// server-side only methods
`unstable_getStaticProps`,`unstable_getStaticPaths`,`unstable_getServerProps`,`unstable_getServerSideProps`]);function decorateSsgExport(t,path,state){const gsspName=state.isPrerender?_constants2.STATIC_PROPS_ID:_constants2.SERVER_PROPS_ID;const gsspId=t.identifier(gsspName);const addGsspExport=path=>{if(state.done){return;}state.done=true;// @ts-ignore invalid return type
const[pageCompPath]=path.replaceWithMultiple([t.exportNamedDeclaration(t.variableDeclaration(// We use 'var' instead of 'let' or 'const' for ES5 support. Since
// this runs in `Program#exit`, no ES2015 transforms (preset env)
// will be ran against this code.
'var',[t.variableDeclarator(gsspId,t.booleanLiteral(true))]),[t.exportSpecifier(gsspId,gsspId)]),path.node]);path.scope.registerDeclaration(pageCompPath);};path.traverse({ExportDefaultDeclaration(path){addGsspExport(path);},ExportNamedDeclaration(path){addGsspExport(path);}});}const isDataIdentifier=(name,state)=>{if(ssgExports.has(name)){if(name===EXPORT_NAME_GET_SERVER_PROPS){if(state.isPrerender){throw new Error(_constants.SERVER_PROPS_SSG_CONFLICT);}state.isServerProps=true;}else{if(state.isServerProps){throw new Error(_constants.SERVER_PROPS_SSG_CONFLICT);}state.isPrerender=true;}return true;}return false;};function nextTransformSsg({types:t}){function getIdentifier(path){const parentPath=path.parentPath;if(parentPath.type==='VariableDeclarator'){const pp=parentPath;const name=pp.get('id');return name.node.type==='Identifier'?name:null;}if(parentPath.type==='AssignmentExpression'){const pp=parentPath;const name=pp.get('left');return name.node.type==='Identifier'?name:null;}if(path.node.type==='ArrowFunctionExpression'){return null;}return path.node.id&&path.node.id.type==='Identifier'?path.get('id'):null;}function isIdentifierReferenced(ident){const b=ident.scope.getBinding(ident.node.name);if(b===null||b===void 0?void 0:b.referenced){// Functions can reference themselves, so we need to check if there's a
// binding outside the function scope or not.
if(b.path.type==='FunctionDeclaration'){return!b.constantViolations.concat(b.referencePaths)// Check that every reference is contained within the function:
.every(ref=>ref.findParent(p=>p===b.path));}return true;}return false;}function markFunction(path,state){const ident=getIdentifier(path);if((ident===null||ident===void 0?void 0:ident.node)&&isIdentifierReferenced(ident)){state.refs.add(ident);}}function markImport(path,state){const local=path.get('local');if(isIdentifierReferenced(local)){state.refs.add(local);}}return{visitor:{Program:{enter(path,state){state.refs=new Set();state.isPrerender=false;state.isServerProps=false;state.done=false;path.traverse({VariableDeclarator(path,state){if(path.node.id.type!=='Identifier'){return;}const local=path.get('id');if(isIdentifierReferenced(local)){state.refs.add(local);}},FunctionDeclaration:markFunction,FunctionExpression:markFunction,ArrowFunctionExpression:markFunction,ImportSpecifier:markImport,ImportDefaultSpecifier:markImport,ImportNamespaceSpecifier:markImport,ExportNamedDeclaration(path,state){const specifiers=path.get('specifiers');if(specifiers.length){specifiers.forEach(s=>{if(isDataIdentifier(s.node.exported.name,state)){s.remove();}});if(path.node.specifiers.length<1){path.remove();}return;}const decl=path.get('declaration');if(decl==null||decl.node==null){return;}switch(decl.node.type){case'FunctionDeclaration':{const name=decl.node.id.name;if(isDataIdentifier(name,state)){path.remove();}break;}case'VariableDeclaration':{const inner=decl.get('declarations');inner.forEach(d=>{if(d.node.id.type!=='Identifier'){return;}const name=d.node.id.name;if(isDataIdentifier(name,state)){d.remove();}});break;}default:{break;}}}},state);if(!state.isPrerender&&!state.isServerProps){return;}const refs=state.refs;let count;function sweepFunction(path){const ident=getIdentifier(path);if((ident===null||ident===void 0?void 0:ident.node)&&refs.has(ident)&&!isIdentifierReferenced(ident)){++count;if(t.isAssignmentExpression(path.parentPath)||t.isVariableDeclarator(path.parentPath)){path.parentPath.remove();}else{path.remove();}}}function sweepImport(path){const local=path.get('local');if(refs.has(local)&&!isIdentifierReferenced(local)){++count;path.remove();if(path.parent.specifiers.length===0){path.parentPath.remove();}}}do{;path.scope.crawl();count=0;path.traverse({// eslint-disable-next-line no-loop-func
