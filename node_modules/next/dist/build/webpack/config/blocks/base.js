...
if(ctx.isDevelopment){if(ctx.isServer||process.platform==='win32'){// Non-eval based source maps are slow to rebuild, so we only enable
...
config.devtool='eval-source-map';}}else{config.devtool=false;}if(!config.module){config.module={rules:[]};}config.module.strictExportPresence=true;return config;});exports.base=base;