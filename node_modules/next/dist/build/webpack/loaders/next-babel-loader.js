...
options.presets=[...options.presets,presetItem];}options.caller.isServer=isServer;options.caller.isModern=isModern;options.caller.isDev=development;options.plugins=options.plugins||[];if(hasReactRefresh){const reactRefreshPlugin=babel.createConfigItem([require('react-refresh/babel'),{skipEnvCheck:true}],{type:'plugin'});options.plugins.unshift(reactRefreshPlugin);}if(!isServer&&isPageFile){const pageConfigPlugin=babel.createConfigItem([require('../../babel/plugins/next-page-config')],{type:'plugin'});options.plugins.push(pageConfigPlugin);}if(isServer&&source.indexOf('next/data')!==-1){const nextDataPlugin=babel.createConfigItem([require('../../babel/plugins/next-data'),{key:(0,_path.basename)(filename)+'-'+(0,_stringHash.default)(filename)}],{type:'plugin'});options.plugins.push(nextDataPlugin);}if(isModern){const nextPreset=options.presets.find(preset=>preset&&preset.value===nextBabelPreset)||{options:{}};const additionalPresets=options.presets.filter(preset=>preset!==nextPreset);const presetItemModern=babel.createConfigItem(nextBabelPresetModern(nextPreset.options),{type:'preset'});options.presets=[...additionalPresets,presetItemModern];}// If the file has `module.exports` we have to transpile commonjs because Babel adds `import` statements
